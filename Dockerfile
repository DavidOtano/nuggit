...
RUN apk add --no-cache bash curl git gcc g++ clang make readline-dev ncurses-dev libc-dev linux-headers p7zip unzip
RUN mkdir /xmake
WORKDIR /xmake
...
RUN xmake config -m release
RUN xmake
RUN cat /nuggit/config.ini | sed 's/\r//g' > /nuggit/build/config.ini

FROM alpine:latest
RUN apk add --no-cache libc-dev linux-headers gcc g++ make
